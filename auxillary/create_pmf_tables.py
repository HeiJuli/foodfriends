...
def create_pmf_tables():
    """Create conditional PMF tables: alpha demographics-only, rho theta-stratified"""
    ...
    # Load and process alpha data (demographics only -- no theta stratification)
    # Rationale: only 54.6% of alpha respondents also have theta; theta-stratifying
    # discards 45% of data and introduces selection bias (esp. 70+ and low-education).
    # Alpha-theta correlation (r=0.14) too weak to justify the data loss.
    ...
    print(f"\n{'='*80}")
    print("PMF TABLE GENERATION")
    print(f"{'='*80}\n")
    print(f"Data loaded:")
    print(f"  Alpha: {len(alpha_clean)} observations (demographics only)")
    print(f"  Rho:   {len(rho_clean)} observations (theta-stratified)")
    print(f"  Theta: {len(theta_clean)} observations (demographics only)")

    # PKL output format with theta stratification
    pmf_tables = {}

    # Process alpha with demographics only (no theta stratification)
    print(f"\nProcessing alpha (demographics only)...")
    grouped_pmf_alpha = (
        alpha_clean.groupby(demo_vars)['alpha']
    ...
    print(f"  Created {len(pmf_dict_alpha)} demographic cells")
    ...
    pmf_tables['_metadata'] = {
        ...
        'stratified_params': ['rho'],
        'unstratified_params': ['alpha', 'theta']
    }
    ...
    print(f"\nSaved PMF tables to ../data/demographic_pmfs.pkl")
    ...
    # Check theta bin distribution for rho
    print(f"\nTheta bin coverage (rho only):")
    theta_bins_present = set(key[-1] for key in pmf_tables['rho'].keys())
    print(f"  rho: {len(theta_bins_present)}/{len(THETA_LABELS)} bins present")
