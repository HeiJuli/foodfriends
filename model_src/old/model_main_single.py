...
class Agent():
    ...
    def _dissonance_modifier_legacy(self, exposure):
        """Legacy: ad-hoc dissonance with exposure gate."""
        raw = max(0, self.theta) if self.diet == "meat" else max(0, -self.theta)
        gate = generic_sigmoid(exposure, 0.25, self.params["w_d"])
        return raw * gate

    # def _dissonance_modifier(self, exposure=None):
    #     """Quadratic dissonance: squared distance between behavior and belief.

    #     d = (diet_numeric - theta)^2   (Galesic et al. 2021, Festinger 1957)

    #     Symmetric for both diet directions; no ad-hoc gating needed since
    #     the social sigmoid in prob_calc already gates by exposure.
    #     """
    #     diet_numeric = 1.0 if self.diet == "veg" else 0.0
    #     return (diet_numeric - self.theta) ** 2
    ...
    def prob_calc(self, other_agent):
       ...
       dissonance = self._dissonance_modifier_legacy(exposure)
       rho_dir = self.rho if self.diet == "meat" else (1 - self.rho)
       print(dissonance)
       dissonance = 1 if exposure < threshold else dissonance
       return rho_dir * kronek*dissonance * social
