...
def mark_agents_with_initial_inertia(model):
    ...
    for agent in model.agents:
        if agent.diet == "meat":
            # Get neighbors from graph directly
...
def run_single_variant(config):
    ...
    # Mark agents with initial inertia AFTER agent_ini
...
if __name__ == '__main__':
    ...
    for i, r in enumerate(results):
        ax1.plot(r['fraction_veg'], label=r['variant_name'],
                alpha=0.8, linewidth=2, color=colors[i])

    ax1.axhline(y=results[0]['initial'], color='gray', linestyle='--',
    ...
    # Zoom on early dynamics (0-10k steps)
    for i, r in enumerate(results):
        ax2.plot(r['fraction_veg'][:10000], label=r['variant_name'],
                alpha=0.8, linewidth=2, color=colors[i])

    ax2.axhline(y=results[0]['initial'], color='gray', linestyle='--',
               alpha=0.3, label='Initial')
    ...
    plt.savefig('visualisations_output/initial_inertia_strategy.png',
                dpi=300, bbox_inches='tight')

    print("\n" + "=" * 70)
    print("Plot saved to visualisations_output/initial_inertia_strategy.png")
    print("=" * 70)
