...
class Agent():
    ...
    def __init__(self, i, params, alpha=None, beta=None, theta=None, diet=None):

        # types can be vegetarian or meat eater
        self.params = params
        self.diet = diet if diet is not None else self.choose_diet(params)
        self.C = self.diet_emissions(self.diet, self.params)
        self.memory = []
        self.i = i
        self.individual_norm = theta if theta is not None else truncnorm.rvs(-1, 1)
        self.global_norm = 0.5
        self.reduction_out = 0
        # implement other distributions (pareto)
        self.alpha = alpha if alpha is not None else self.choose_alpha_beta(params["alpha"])
        self.beta = beta if beta is not None else self.choose_alpha_beta(params["beta"])
...
#%% Model 
class Model():
    def __init__(self, params, survey_data=None):
    ...
    def agent_ini(self, params):
        """ Initialize agents with survey data when available."""
        self.agents = []
        
        for i, node in enumerate(self.G1.nodes()):
            if self.survey_data is not None and i<len(self.survey_data):
                agent_params = self.survey_data.iloc[i].to_dict()
            else:
                agent_params = {"alpha": params["alpha"], "beta": params["beta"], "theta": 0.5}  # Defaults
        
            # Create agent with these parameters
            agent = Agent(i, params, **agent_params)
